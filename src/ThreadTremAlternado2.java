...
public class ThreadTremAlternado2 extends Thread {

  private Button button;
  private String name;
  public int speed = 2;
  private volatile boolean interrompido4 = false;
  ...
  public ThreadTremAlternado2(Button trem, String name, int opcaoDeParada, Peterson peterson) {
    this.button = trem;
    this.name = name;
  ...
  public void run() {
    ...
    if (button.getId().equals("TremAmarelo")) {
      ...
      while (button.getLayoutX() > 1) {
        ...
        // funcao para controlar a velocidade
        sleep1();
      }
    }// fim do if do trem amarelo
  ...
  public void pararTremVerde() {
    if (button.getLayoutX() > 182 && button.getLayoutX() < 215) {
      Platform.runLater(() -> button.setLayoutY(button.getLayoutY() + 1));
    } else if (button.getLayoutX() > 338 && button.getLayoutX() < 378) {
      Platform.runLater(() -> button.setLayoutY(button.getLayoutY() - 1));
    } else if (button.getLayoutX() > 500 && button.getLayoutX() < 536) {
      Platform.runLater(() -> button.setLayoutY(button.getLayoutY() + 1));
    } else if (button.getLayoutX() > 650 && button.getLayoutX() < 688) {
      Platform.runLater(() -> button.setLayoutY(button.getLayoutY() - 1));
    }
  }
  ...
  public void movimentarTremVerdeVariavelDeTravamento() {
    // condicionais para fazer o trem verde se mover
    if (button.getId().equals("TremVerde")) {
      if (button.getLayoutX() > 182 && button.getLayoutX() < 218) {
        if (tunel1amarelo == 1) {
          button.setLayoutX(button.getLayoutX() - 1);
          button.setLayoutY(button.getLayoutY() + 1);
        } else {
  ...
  public void pararThreadsAlternado2() {
    interrompido4 = true;
  }

  public void reiniciarInterrompido4() {
    interrompido4 = false;
  }
